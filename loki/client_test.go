...
func TestClient_Handle(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			err := serverURL.Set("htp://10.0.49.31:9096")
...
func TestClient_EncodeJSON(t *testing.T) {
	c, err := NewWithDefault("http://10.0.49.31:3200/loki/api/v1/push")

	if err != nil {
		fmt.Println("no error")
	}
	//_, e := c.send(context.Background(), "ssss", []byte("{\"aa\":\"bb\"}"))
	//c.Handle(model.LabelSet{"foo": "bar"}, time.Now(), "**********")
	...
	c.SendBatch("8888",
		NewBatch(
			Entry{Labels: model.LabelSet{"foo": "bar"}, Entry: logproto.Entry{Timestamp: time.Now(), Line: "11888"}},
			Entry{Labels: model.LabelSet{"foo": "bar"}, Entry: logproto.Entry{Timestamp: time.Now(), Line: "22888"}},
			Entry{Labels: model.LabelSet{"foo": "buzz"}, Entry: logproto.Entry{Timestamp: time.Now(), Line: "8888"}},
			Entry{Labels: model.LabelSet{"foo": "buzz"}, Entry: logproto.Entry{Timestamp: time.Now(), Line: "99999"}},
		),
	)
}
